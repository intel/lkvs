...
source ../guest-test/test_params.py
...
source ../guest-test/guest.test_executor.sh
...
EOF
    ...
    grep idxd-portal /proc/interrupts
EOF
...
EOF
    ...
    grep "00:01.0" /sys/kernel/debug/iommu/intel/ir_translation_struct

    host_cpu_assigned=$(tail -n1 /sys/kernel/debug/tracing/trace | awk -F ' ' '{print $17}' | awk -F ',' '{print $1}')
    host_vector_assigned=$(tail -n1 /sys/kernel/debug/tracing/trace | awk -F ' ' '{print $21}' | awk -F ',' '{print $1}')
    host_vector_assigned=$((host_vector_assigned)) # hex to decimal
    #host_vector_in_irte=$(grep "00:01.0" /sys/kernel/debug/iommu/intel/ir_translation_struct | awk 'END {print}' | awk -F ' ' '{print $5}')
