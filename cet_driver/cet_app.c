...
{
	cpu_set_t set;
	unsigned long ssp_addr, msr_shstk = 0;
	unsigned long *ssp, *bp;
	...
	asm("movq %%rbx,%0" : "=r"(ssp));
	asm("movq %%rbp,%0" : "=r"(bp));

	if (ssp_addr) {
		printf("ssp:%lx is no 0, *ssp:%lx\n", ssp_addr,
		       *(unsigned long *)ssp_addr);
		printf("bp+1:%p, *(bp+1):0x%lx, ssp:%p, *ssp:0x%lx\n",
		       bp + 1, *(bp + 1), ssp, *ssp);
		msr_shstk = system("rdmsr -a 0x6a7");
	} else {
		printf("ssp:%lx is 0, test msr should 0\n", ssp_addr);
		msr_shstk = system("rdmsr -a 0x6a7");
		printf("msr_shstk:%lx\n", msr_shstk);
	}
